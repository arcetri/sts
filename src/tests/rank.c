...
Rank_metric_print(struct state *state, long int sampleCount, long int toolow, long int *freqPerBin)
{
	...
	if (state->legacy_output == true) {
		...
		if (proportion_passed == false && uniformity_passed == false) {
			state->metric_results.rank = FAILED_BOTH;
		} else if (proportion_passed == false) {
			state->metric_results.rank = FAILED_PROPORTION;
		} else if (uniformity_passed == false) {
			state->metric_results.rank = FAILED_UNIFORMITY;
		} else {
			state->metric_results.rank = PASSED_BOTH;
