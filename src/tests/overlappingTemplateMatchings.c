...
OverlappingTemplateMatchings_init(struct state *state)
{
	 ...
	 */
	if (state == NULL) {
		err(140, __func__, "state arg is NULL");
	}
	...
	if (state->cSetup != true) {
		err(140, __func__, "test constants not setup prior to calling %s for %s[%d]",
	...
	if (state->driver_state[test_num] != DRIVER_NULL && state->driver_state[test_num] != DRIVER_DESTROY) {
		err(140, __func__, "driver state %d for %s[%d] != DRIVER_NULL: %d and != DRIVER_DESTROY: %d",
	...
	if (m != DEFAULT_OVERLAPPING) {
		...
		warn(__func__, "disabling test %s[%d]: requires number of blocks (N) * min_pi: %f > %d. "
 ...
 */
void
OverlappingTemplateMatchings_iterate(struct thread_state *thread_state)
{
	...
	if (thread_state == NULL) {
		err(141, __func__, "thread_state arg is NULL");
	}
	struct state *state = thread_state->global_state;
	if (state == NULL) {
		err(141, __func__, "state arg is NULL");
	}
	if (state->testVector[test_num] != true) {
	...
	if (state->epsilon == NULL) {
		err(141, __func__, "state->epsilon is NULL");
	}
	if (state->epsilon[thread_state->thread_id] == NULL) {
		err(141, __func__, "state->epsilon[%ld] is NULL", thread_state->thread_id);
	}
	if (state->driver_state[test_num] != DRIVER_INIT && state->driver_state[test_num] != DRIVER_ITERATE) {
		err(141, __func__, "driver state %d for %s[%d] != DRIVER_INIT: %d and != DRIVER_ITERATE: %d",
	...
	for (i = 0; i < stat.N; i++) {
		...
		for (j = 0; j < BLOCK_LENGTH_OVERLAPPING - m + 1; j++) {
			match = true;
			for (k = 0; k < m; k++) {
				if (B_VALUE != state->epsilon[thread_state->thread_id][i * BLOCK_LENGTH_OVERLAPPING + j + k]) {
					match = false;
					break;
				}
			}
					...
					double p_value)
{
	 ...
	 */
	if (stream == NULL) {
		err(142, __func__, "stream arg is NULL");
	}
	if (state == NULL) {
		err(142, __func__, "state arg is NULL");
	}
	if (stat == NULL) {
		err(142, __func__, "stat arg is NULL");
	}
	if (p_value == NON_P_VALUE && stat->success == true) {
		err(142, __func__, "p_value was set to NON_P_VALUE but stat->success == true");
	}
...
OverlappingTemplateMatchings_print_p_value(FILE * stream, double p_value)
{
	 ...
	 */
	if (stream == NULL) {
		err(143, __func__, "stream arg is NULL");
	}
...
OverlappingTemplateMatchings_print(struct state *state)
{
	 ...
	 */
	if (state == NULL) {
		err(144, __func__, "state arg is NULL");
	}
	if (state->testVector[test_num] != true) {
	...
	if (state->datatxt_fmt[test_num] == NULL) {
		err(144, __func__, "format for data0*.txt filename is NULL");
	}
	if (state->driver_state[test_num] != DRIVER_ITERATE) {
		err(144, __func__, "driver state %d for %s[%d] != DRIVER_ITERATE: %d",
	...
	for (i = 0; i < state->stats[test_num]->count; ++i) {
		...
		if (ok == false) {
			errp(144, __func__, "error in writing to %s", stats_txt);
		}
		...
		if (ok == false) {
			errp(144, __func__, "error in writing to %s", results_txt);
		}
	}
	...
	io_ret = fflush(stats);
	if (io_ret != 0) {
		errp(144, __func__, "error flushing to: %s", stats_txt);
	}
	errno = 0;		// paranoia
	io_ret = fclose(stats);
	if (io_ret != 0) {
		errp(144, __func__, "error closing: %s", stats_txt);
	}
	free(stats_txt);
	stats_txt = NULL;
	...
	if (io_ret != 0) {
		errp(144, __func__, "error flushing to: %s", results_txt);
	}
	errno = 0;		// paranoia
	io_ret = fclose(results);
	if (io_ret != 0) {
		errp(144, __func__, "error closing: %s", results_txt);
	}
	free(results_txt);
	results_txt = NULL;
		...
		for (j = 0; j < state->partitionCount[test_num]; ++j) {
			...
			if (j < state->p_val[test_num]->count) {
				for (i = j; i < state->p_val[test_num]->count; i += state->partitionCount[test_num]) {
					...
					if (ok == false) {
						errp(144, __func__, "error in writing to %s", data_txt);
					}
			...
			io_ret = fflush(data);
			if (io_ret != 0) {
				errp(144, __func__, "error flushing to: %s", data_txt);
			}
			errno = 0;	// paranoia
			io_ret = fclose(data);
			if (io_ret != 0) {
				errp(144, __func__, "error closing: %s", data_txt);
			}
			free(data_txt);
			data_txt = NULL;
		}
...
OverlappingTemplateMatchings_metric_print(struct state *state, long int sampleCount, long int toolow, long int *freqPerBin)
{
	 ...
	 */
	if (state == NULL) {
		err(145, __func__, "state arg is NULL");
	}
	if (freqPerBin == NULL) {
		err(145, __func__, "freqPerBin arg is NULL");
	}
	...
	if (state->legacy_output == true) {
		...
		if (io_ret != 0) {
			errp(145, __func__, "error flushing to: %s", state->finalReptPath);
		}
		...
		if (proportion_passed == false && uniformity_passed == false) {
			state->metric_results.overlapping = FAILED_BOTH;
		} else if (proportion_passed == false) {
			state->metric_results.overlapping = FAILED_PROPORTION;
		} else if (uniformity_passed == false) {
			state->metric_results.overlapping = FAILED_UNIFORMITY;
		} else {
			state->metric_results.overlapping = PASSED_BOTH;
...
OverlappingTemplateMatchings_metrics(struct state *state)
{
	 ...
	 */
	if (state == NULL) {
		err(146, __func__, "state arg is NULL");
	}
	if (state->testVector[test_num] != true) {
	...
	if (state->driver_state[test_num] != DRIVER_PRINT && state->resultstxtFlag == true) {
		err(146, __func__, "driver state %d for %s[%d] != DRIVER_PRINT: %d",
		...
		err(146, __func__, "driver state %d for %s[%d] != DRIVER_ITERATE: %d",
		    state->driver_state[test_num], state->testNames[test_num], test_num, DRIVER_ITERATE);
	}
	...
	if (freqPerBin == NULL) {
		errp(146, __func__, "cannot malloc of %ld elements of %ld bytes each for freqPerBin",
...
OverlappingTemplateMatchings_destroy(struct state *state)
{
	 ...
	 */
	if (state == NULL) {
		err(147, __func__, "state arg is NULL");
	}
	if (state->testVector[test_num] != true) {
