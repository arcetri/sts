...
ApproximateEntropy_init(struct state *state)
{
	 ...
	 */
	if (state == NULL) {
		err(10, __func__, "state arg is NULL");
	}
	 ...
	 */
	if (m >= lround(floor(state->c.logn / state->c.log2 - 5.0))) {
		warn(__func__, "disabling test %s[%d]: requires block length(m): %ld >= %ld",
		     state->testNames[test_num], test_num, m, lround(floor(state->c.logn / state->c.log2 - 5.0)));
...
compute_phi(struct thread_state *thread_state, long int blocksize)
{
	...
	if (blocksize > (BITS_N_LONGINT - 1)) {	// firewall
		err(18, __func__, "m is too large, 1 << (m:%ld) can't be longer than %ld bits", blocksize, BITS_N_LONGINT - 1);
	}
	if (state->apen_C == NULL) {
	...
	if (powLen > state->apen_C_len) {
		err(18, __func__, "powLen: %ld is too large, "
...
ApproximateEntropy_print_stat(FILE * stream, struct state *state, struct ApproximateEntropy_private_stats *stat, double p_value)
{
	 ...
	 */
	if (stream == NULL) {
		err(12, __func__, "stream arg is NULL");
	}
	if (state == NULL) {
		err(12, __func__, "state arg is NULL");
	}
	if (stat == NULL) {
		err(12, __func__, "stat arg is NULL");
	}
	if (state->cSetup != true) {
		err(12, __func__, "test constants not setup prior to calling %s for %s[%d]",
		    __func__, state->testNames[test_num], test_num);
	}
	if (p_value == NON_P_VALUE && stat->success == true) {
		err(12, __func__, "p_value was set to NON_P_VALUE but stat->success == true");
	}
...
ApproximateEntropy_print_p_value(FILE * stream, double p_value)
{
	 ...
	 */
	if (stream == NULL) {
		err(13, __func__, "stream arg is NULL");
	}
...
ApproximateEntropy_print(struct state *state)
{
	 ...
	 */
	if (state == NULL) {
		err(14, __func__, "state arg is NULL");
	}
	if (state->testVector[test_num] != true) {
	...
	for (i = 0; i < state->stats[test_num]->count; ++i) {
		...
		if (ok == false) {
			errp(14, __func__, "error in writing to %s", stats_txt);
		}
		...
		if (ok == false) {
			errp(14, __func__, "error in writing to %s", results_txt);
		}
	}
	...
	io_ret = fflush(stats);
	if (io_ret != 0) {
		errp(14, __func__, "error flushing to: %s", stats_txt);
	}
	errno = 0;		// paranoia
	io_ret = fclose(stats);
	if (io_ret != 0) {
		errp(14, __func__, "error closing: %s", stats_txt);
	}
	free(stats_txt);
	stats_txt = NULL;
	...
	if (io_ret != 0) {
		errp(14, __func__, "error flushing to: %s", results_txt);
	}
	errno = 0;		// paranoia
	io_ret = fclose(results);
	if (io_ret != 0) {
		errp(14, __func__, "error closing: %s", results_txt);
	}
	free(results_txt);
	results_txt = NULL;
	...
	if (state->partitionCount[test_num] > 1) {
		for (j = 0; j < state->partitionCount[test_num]; ++j) {
			...
			if (snprintf_ret <= 0 || snprintf_ret >= BUFSIZ || errno != 0) {
				errp(14, __func__, "snprintf failed for %d bytes for data%03ld.txt, returned: %d", BUFSIZ,
				     j + 1, snprintf_ret);
			}
			...
			if (j < state->p_val[test_num]->count) {
				for (i = j; i < state->p_val[test_num]->count; i += state->partitionCount[test_num]) {
					...
					if (ok == false) {
						errp(14, __func__, "error in writing to %s", data_txt);
					}
			...
			io_ret = fflush(data);
			if (io_ret != 0) {
				errp(14, __func__, "error flushing to: %s", data_txt);
			}
			errno = 0;	// paranoia
			io_ret = fclose(data);
			if (io_ret != 0) {
				errp(14, __func__, "error closing: %s", data_txt);
			}
			free(data_txt);
			data_txt = NULL;
		}
...
ApproximateEntropy_metric_print(struct state *state, long int sampleCount, long int toolow, long int *freqPerBin)
{
	 ...
	 */
	if (state == NULL) {
		err(15, __func__, "state arg is NULL");
	}
	if (freqPerBin == NULL) {
		err(15, __func__, "freqPerBin arg is NULL");
	}
	...
	if (state->legacy_output == true) {
		...
		if (io_ret != 0) {
			errp(15, __func__, "error flushing to: %s", state->finalReptPath);
		}
		...
		if (proportion_passed == false && uniformity_passed == false) {
			state->metric_results.approximate_entropy = FAILED_BOTH;
		} else if (proportion_passed == false) {
			state->metric_results.approximate_entropy = FAILED_PROPORTION;
		} else if (uniformity_passed == false) {
			state->metric_results.approximate_entropy = FAILED_UNIFORMITY;
		} else {
			state->metric_results.approximate_entropy = PASSED_BOTH;
...
ApproximateEntropy_metrics(struct state *state)
{
	 ...
	 */
	if (state == NULL) {
		err(16, __func__, "state arg is NULL");
	}
	if (state->testVector[test_num] != true) {
	...
	if (freqPerBin == NULL) {
		errp(16, __func__, "cannot malloc of %ld elements of %ld bytes each for freqPerBin",
...
ApproximateEntropy_destroy(struct state *state)
{
	 ...
	 */
	if (state == NULL) {
		err(17, __func__, "state arg is NULL");
	}
	if (state->testVector[test_num] != true) {
