...
Frequency_init(struct state *state)
{
	...
	if (state->cSetup != true) {
		err(70, __FUNCTION__, "test constants not setup prior to calling %s for %s[%d]",
	...
	state->stats[test_num] =
	    create_dyn_array(sizeof(struct Frequency_private_stats), DEFAULT_CHUNK, state->tp.numOfBitStreams, false);
...
{
	struct Frequency_private_stats stat;	// stats for this interation
	...
	if (state->testVector[test_num] != true) {
		dbg(DBG_LOW, "interate driver interface for %s[%d] called when test vector was false",
		    state->testNames[test_num], test_num);
		return;
	}
	...
	stat.sum = 0;
	for (i = 0; i < n; i++) {
		stat.sum += 2 * (int) state->epsilon[i] - 1;
	}
	s_obs = fabs((double) stat.sum) / state->c.sqrtn;
...
/*
 * Frequency_print - print to results.txt, data*.txt, stats.txt for all interations
 *
 * given:
 *      state           // run state to test under
 *
...
{
	struct Frequency_private_stats *stat;	// pointer to statistics of an interation
	double p_value;		// p_value interation test result(s)
