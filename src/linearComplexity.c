...
LinearComplexity_init(struct state *state)
{
	...
	if (state->linear_B_ == NULL) {
		errp(10, __FUNCTION__, "cannot malloc of %ld elements of %ld bytes each for state->linear_B_",
	...
	if (state->linear_C == NULL) {
		errp(10, __FUNCTION__, "cannot malloc of %ld elements of %ld bytes each for state->linear_C",
		     state->tp.linearComplexitySequenceLength, sizeof(BitSequence));
	}
	state->linear_P = malloc(state->tp.linearComplexitySequenceLength * sizeof(state->linear_P[0]));
	if (state->linear_P == NULL) {
		errp(10, __FUNCTION__, "cannot malloc of %ld elements of %ld bytes each for state->linear_P",
		     state->tp.linearComplexitySequenceLength, sizeof(BitSequence));
	}
	state->linear_T = malloc(state->tp.linearComplexitySequenceLength * sizeof(state->linear_T[0]));
	if (state->linear_T == NULL) {
		errp(10, __FUNCTION__, "cannot malloc of %ld elements of %ld bytes each for state->linear_T",
		     state->tp.linearComplexitySequenceLength, sizeof(BitSequence));
	}
...
LinearComplexity_iterate(struct state *state)
{
	...
	if (state->linear_B_ == NULL) {
		err(10, __FUNCTION__, "state->linear_B_ is NULL");
	}
	if (state->linear_C == NULL) {
		err(10, __FUNCTION__, "state->linear_C is NULL");
	}
	if (state->linear_P == NULL) {
		err(10, __FUNCTION__, "state->linear_P is NULL");
	}
	if (state->linear_T == NULL) {
		err(10, __FUNCTION__, "state->linear_T is NULL");
	}
