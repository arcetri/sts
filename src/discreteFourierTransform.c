...
DiscreteFourierTransform_init(struct state *state)
{
	...
	if (state->fft_X == NULL) {
		errp(10, __FUNCTION__, "cannot malloc of %ld elements of %ld bytes each for state->fft_X",
	...
	if (state->fft_wsave == NULL) {
		errp(10, __FUNCTION__, "cannot malloc of %ld elements of %ld bytes each for state->fft_wsave",
	...
	if (state->fft_m == NULL) {
		errp(10, __FUNCTION__, "cannot malloc of %ld elements of %ld bytes each for state->fft_m",
...
DiscreteFourierTransform_iterate(struct state *state)
{
	...
	if (state->fft_X == NULL) {
		err(10, __FUNCTION__, "state->fft_X is NULL");
	}
	if (state->fft_wsave == NULL) {
		err(10, __FUNCTION__, "state->fft_wsave is NULL");
	}
	if (state->fft_m == NULL) {
		err(10, __FUNCTION__, "state->fft_m is NULL");
	}
	if (state->cSetup != true) {
		err(10, __FUNCTION__, "test constants not setup prior to calling %s for %s[%d]",
		    __FUNCTION__, state->testNames[test_num], test_num);
	...
	X[n] = 0;		// fix valgrind detected issue (see below)
