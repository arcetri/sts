...
static void displayBits(FILE * fp, unsigned int *A, unsigned long long value,
			long int count, long int *nonPeriodic);
...
main(int argc, char *argv[])
{
	...
	usage_err(usage, 1, __FUNCTION__,
		  "expected 4 argumentsi, found only %d", argc);
    }
    errno = 0;
    ...
    if (A == NULL) {
	errp(1, __FUNCTION__,
	     "cannot calloc %ld unsigned int of %ld bytes each", B,
	     sizeof(unsigned int));
    }
...
static void
displayBits(FILE * fp, unsigned int *A, unsigned long long value,
	    long int count, long int *nonPeriodic)
{
